// Apply Gausian along with size reduction
#include "stdio.h"
#include "opencv/cv.h"
...
IplImage* doPyrDown(IplImage* in,int filter = IPL_GAUSSIAN_5x5) {
   ...
   IplImage* out2 = cvCreateImage(cvSize( in->width/2, in->height/2 ),in->depth,in->nChannels);
   cvPyrDown( in, out1 );   
   cvSmooth( out1, out2, CV_GAUSSIAN, 9, 9 );
   return( out2 );
};
